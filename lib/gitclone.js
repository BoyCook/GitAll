...
function GitClone(user, targetDir, protocol) {
	this.user = user;
...
GitClone.prototype.validate = function() {
	if (!this.isDir(this.targetDir)) {
		console.log('Target directory [%s] does not exist, creating.', this.targetDir);
		//TODO: make dir and exit if there's an error
	}
	//TODO: check protocol matches available URLs [ssh|https|svn]
...
GitClone.prototype.updateRepo = function(repo) {
	...
	this.doSpawn('sh', [__dirname + '/pull.sh', repo, this.urls.ssh, this.urls.https], this.logToFile);
};
...
GitClone.prototype.cloneRepo = function(repo) {
	...
	if (this.isDir(repoDir)) {
		console.log('Not cloning repo [%s] it already exists at [%s]', repo.name, repoDir);
	} else {
...
GitClone.prototype.generateRepoURL = function(repo) {
	//TODO: use string formatter here
	if (this.protocol === 'ssh') {
		return this.urls.ssh + repo + '.git';
	} else if (this.protocol === 'https') {
		return this.urls.https + repo;
	}
};

GitClone.prototype.getRepos = function(success) {
	var context = this;
	var url = this.urls.api + '/users/' + this.user +  '/repos';
