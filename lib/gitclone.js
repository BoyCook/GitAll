...
GitClone.prototype.getRepoStatus = function(repo) {
	this.doSpawn('sh', ['./bin/status.sh', repo], this.logToConsole);
};
...
GitClone.prototype.updateRepo = function(repo) {
	...
	this.doSpawn('sh', ['pull.sh', repo], this.logToFile);
};
...
GitClone.prototype.cloneRepo = function(repo) {
	...
	this.doSpawn('sh', ['clone.sh', this.targetDir, repoURL], this.logToFile);	
};
